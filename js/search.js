...
EPUBJS.reader.plugins.SearchController = function(Book) {
	...
	var query = function() {
		...
		$searchResults.empty();
		$searchResults.append("<li><p>Buscando...</p></li>");
		...
		EPUBJS.reader.search.request(q, function(data) {
			...
			if(results.length == 0) {
				$searchResults.append("<li><p>Nada encontrado</p></li>");
				return;
			}
	...
	$searchBox.on("search", function(e) {
		...
		if(q == '') {
			$searchResults.empty();
			if(reader.SidebarController.getActivePanel() == "Busca") {
		...
		reader.SidebarController.changePanelTo("Busca");
